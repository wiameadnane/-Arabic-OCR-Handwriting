...
# -------------------------------
# Rebuild the dictionary (should match the training one)
...
def decode_ctc_beam_search(y_pred, beam_width=10):
    input_length = np.array([y_pred.shape[1]])
    y_pred_log = tf.math.log(y_pred + 1e-8)
    y_pred_transposed = tf.transpose(y_pred_log, [1, 0, 2])  # (max_time, batch, num_classes)
    ...
    dense_decoded = tf.sparse.to_dense(decoded[0], default_value=-1).numpy()[0]
    filtered = [c for c in dense_decoded if c != -1 and c != blank_index]
    out_str = '|'.join([codes_to_chars.get(c, '') for c in filtered])
    return out_str
...
# Rebuild inference model from it
predict_model = Model(inputs=model.inputs[0],  # First input = image
                      outputs=model.get_layer('softmax').output)


def infer_image2(image_path, method='beam', beam_width=30):
    img = get_image(image_path)
    img = np.expand_dims(img, axis=0)  # shape: (1, 100, 300, 1)
    y_pred = predict_model.predict(img)
    y_pred = y_pred[:, :, :120]  # slice to valid characters

    if method == 'beam':
        decoded = decode_ctc_beam_search(y_pred, beam_width=beam_width)
    else:
        decoded = ctc_decode(y_pred)  # greedy by default
